	...
	<div class="bg">
		<div class="info">
			<div class="body">
				...
				<div class="body_">
					<span></span>
					<span>{{new Date(res.time*1000).toLocaleString()}}</span>
		...
		<div class="foot">
			<ul>
				<li class="left_hr" @click="del">删除</li>
			</ul>
		</div>
	</div>
</template>
    ...
    methods: {
		del:function(){
			const self=this;
				...
				axios({
				    method: 'post',
				    url: self.GLOBAL.baseURL+'Notice/del',
				    data: Qs.stringify(data)
				}).then(function (response) {
						console.log(response);
					    if(response.data.code == '1'){
						...
						self.$router.push({ path: '/notice/n_detail' })
					    );										
					    }
					  })
					  .catch(function (error) {
